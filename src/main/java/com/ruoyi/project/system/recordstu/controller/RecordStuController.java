...
public class RecordStuController extends BaseController
{
    ...
    {
        RecordStu recordStu = recordStuService.selectRecordStuById(testStuId);
    ...
    {
        recordStuService.updateTestStu(recordStu);

        if(recordStu.getPassOrNot()==1) {
            int  temp=recordStuService.insertRecordStu(recordStu);
            return toAjax(temp);
        }
        return toAjax(1);
    }
